...
public class SqlHandle {
	...
	public SqlHandle CONDITION_CUSTOM(String condition){
	...
	public SqlHandle ORDERBY(String order, String field){
		...
		//value.getClass().getTypeName();
		String nOrder =field+" "+order;
		//如果是String类型，需要加上单引号。
		this.order.add(nOrder);


		//this.order=" "+field+" "+order;
		return this;
	}
	public SqlHandle ORDERBY_CUSTOM(String order){
	...
	private void addorder(){
		if(order.size()>0){
			...
			for (String nStr : order) {
				if(!nStr.equals(order.get(order.size()-1))){
					sql.append(" "+nStr+" , ");
				}else{
					sql.append(" "+nStr);
				}
