...
int http_ContentMD5( http_t *conn ,char *Etag)
{
    char *i;
    memset(Etag,0,ETAG_LEN);
    if( ( i = http_header( conn, "Content-MD5:" ) ) == NULL )
    {
        return( -2 );
    }
    if(i[0]==' ')
    {
        strncpy(Etag,&i[1],ETAG_LEN-1);
    }
    else
        memset(Etag,0,ETAG_LEN);
    Etag[ETAG_LEN-1]=0;
...
int http_Etag( http_t *conn ,char *Etag)
{
    char *i;
    memset(Etag,0,ETAG_LEN);
    if(!http_ContentMD5(conn ,Etag))
        return 0;
    if( ( i = http_header( conn, "ETag:" ) ) == NULL )
    {
        return( -2 );
    }
    if((i[0]==' ')&&(i[1]=='\"')&&(i[34]=='\"'))
    {
        strncpy(Etag,&i[2],ETAG_LEN-1);
    }
    else
        memset(Etag,0,ETAG_LEN);
    Etag[ETAG_LEN-1]=0;
...
int http_connect( http_t *conn, int proto, char *proxy, char *host, int port, char *user, char *pass )
{
    ...
    //printf("=============%s %s %d: conn->host = %s\n", __FILE__, __func__, __LINE__, conn->host);
    printf("=============%s %s %d: host = %s, port = %d\n", __FILE__, __func__, __LINE__, host, port);
    //printf("=============%s %s %d: conn->local_if = %s\n", __FILE__, __func__, __LINE__, conn->local_if);
    ...
    if( ( conn->fd = tcp_connect( host, port, conn->local_if ) ) == -1 )
    {
        ...
        printf("==========%s %d: Unable to connect to server %s:%i\n", __FILE__, __LINE__, host, port);
        return( 0 );
    }
...
int http_exec( http_t *conn )
{
...
#endif
    http_addheader( conn, "" );
    //printf("===================%s %d: conn->fd = %d, conn->request = %s\n", __FILE__, __LINE__, conn->fd, conn->request);
    ...
    while( 1 )
    {
        if( (ret = read( conn->fd, s, 1 )) <= 0 )
        {
            /* We'll put the message in conn->headers, not in request */
            sprintf( conn->headers, _("Connection gone.\n") );
            printf("===================%s %d: ret = %d, return\n", __FILE__, __LINE__, ret);
            return( 0 );
        }
        if( *s == '\r' )
        {
...
//	printf("%s",conn->headers);
#ifdef DEBUG
    fprintf( stderr, "--- Reply headers ---\n%s--- End of headers ---\n", conn->headers );
#endif
    //printf("===============%s %d: conn->headers = %s", __FILE__, __LINE__, conn->headers);
...
char *http_header( http_t *conn, char *header )
{
	...
	if(strncmp(conn->headers, "Connection gone.", strlen("Connection gone.")) == 0)
	{
		return( NULL );
	}
	//printf("============%s %d: headers = %s\n", __FILE__, __LINE__, conn->headers);
	for( i = 1; conn->headers[i] != '\0'; i ++ )
	{		
		//printf("============%s %d: conn->headers[i-1] = %c\n", __FILE__, __LINE__, conn->headers[i-1]);
	    if( conn->headers[i-1] == '\n' )
	    {
			//printf("============%s %d\n", __FILE__, __LINE__);
