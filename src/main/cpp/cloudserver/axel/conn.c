...
int conn_set( conn_t *conn, char *set_url )
{
    ...
    else
    {
        *i = 0;
        ...
        //printf("====================%s %s %d: conn->dir = %s\n", __FILE__, __func__, __LINE__, conn->dir);
    }
    ...
    *i = 0;
    ...
    //printf("====================%s %s %d: conn->file = %s\n", __FILE__,  __func__,__LINE__, conn->file);
    //printf("====================%s %s %d: conn->dir = %s\n", __FILE__, __func__, __LINE__, conn->dir);
...
int conn_info( conn_t *conn )
{
    ...
    if( conn->proto == PROTO_FTP && !conn->proxy )
    {
        ...
        if( !ftp_cwd( conn->ftp, conn->dir ) )
            return( 0 );
        conn->Fsize=conn->size = ftp_size( conn->ftp, conn->file, MAX_REDIR );
    ...
    else
    {
		...
		printf("============%s %d: conn->size = %lld, status = %d\n", __FILE__, __LINE__, conn->size, conn->http->status);

        if( conn->http->status == 206 && conn->size >= 0 )
        {
            conn->supported = 1;
            conn->size ++;
            conn->Fsize=conn->size;
        }
        else if( conn->http->status == 200 || conn->http->status == 206 )
        {
            conn->Fsize=conn->size;
            printf("3 conn->size: %lld\n",conn->size);
            conn->supported = 0;
            conn->size = INT_MAX;
        }
        else
        ...
        http_Etag( conn->http , conn->MD5);	//MD5 chenlin
        //printf("==============%s %d: conn->MD5 = %s\n", __FILE__, __LINE__, conn->MD5);
